# COPSOQ II — Redesenho da Base de Dados: Plano de Implementação

> **Para quem executa:** SUB-SKILL RECOMENDADA: use `superpowers:executing-plans` para seguir tarefa por tarefa. Os passos usam `- [ ]` para marcar progresso.
>
> **Spec de referência:** [docs/superpowers/specs/2026-04-10-copsoq-database-redesign.md](../specs/2026-04-10-copsoq-database-redesign.md)

**Goal:** Conectar a coleta do questionário à análise: todas as respostas que o colaborador envia via QR code devem alimentar direto as views do dashboard e o PDF do PGR, usando a lógica COPSOQ II correta (conversão 1-5, inversão de itens, classificação NR-01).

**Arquitetura:** Uma única cadeia — `insert_survey_response → respondents/responses → views analíticas (1-5) → views-ponte (0-100) → dashboard.ts → UI`. Substitui o schema bagunçado atual (duas arquiteturas paralelas desconectadas) por uma migração única em SQL. Front-end só muda 3 arquivos (`survey.ts`, `SurveyPage.tsx`, `empresas.ts`).

**Tech Stack:** Supabase (PostgreSQL), React 19 + TypeScript + Vite, Tailwind, Supabase JS v2.
...
|------|---------|------------------|
| Create | `sql/2026-04-10-copsoq-redesign.sql` | Migração única: DROP do legado + CREATE tabelas/views/função/RLS + seeds |
| Modify | `src/services/survey.ts` | `fetchQuestions` usa novo schema; `submitSurveyResponse` chama RPC |
| Modify | `src/pages/SurveyPage.tsx` | Trocar `category_name` por `subscale` (1 linha) |
| Modify | `src/services/empresas.ts` | Reescrever 2 funções que liam das tabelas dropadas |

**Sobre testes:** o projeto não tem test runner (só `dev`, `build`, `preview` no package.json). A verificação usa:
- `npm run build` — garante tipos OK e que nada compila quebrado
- SELECTs de sanidade no SQL Editor do Supabase — confirmam que as views retornam o shape esperado
- Smoke test manual no browser — abrir a survey, responder, ver o dashboard

---

## Task 1: Criar o arquivo de migração SQL — estrutura base

**Files:**
- Create: `sql/2026-04-10-copsoq-redesign.sql`

- [ ] **Step 1: Criar o arquivo com o bloco completo de DROP + CREATE TABLE + views + função + RLS (sem os seeds)**

Usar a ferramenta Write para criar `sql/2026-04-10-copsoq-redesign.sql` com este conteúdo exato:
...
-- Os seeds (76 perguntas + 29 pgr_actions) serão adicionados nas tasks 2 e 3.
```

- [ ] **Step 2: Verificar que o arquivo foi criado**

Ler o arquivo com Read. Esperado: ~300 linhas, começando com `-- M.A.P.A. — Redesenho COPSOQ II`.

---

## Task 2: Adicionar o seed das 76 perguntas

**Files:**
- Modify: `sql/2026-04-10-copsoq-redesign.sql` (append at end)

- [ ] **Step 1: Acrescentar o bloco de INSERTs ao final do arquivo**

Usar Edit. Localizar a linha `-- Os seeds (76 perguntas + 29 pgr_actions) serão adicionados nas tasks 2 e 3.` e substituir pelo bloco abaixo (mantém a linha como cabeçalho):
...
## Task 3: Adicionar o seed das 29 `pgr_actions`

**Files:**
- Modify: `sql/2026-04-10-copsoq-redesign.sql` (append at end)

- [ ] **Step 1: Acrescentar o bloco das 29 ações de controle ao final do arquivo**

Usar Edit para inserir este bloco **depois do bloco de INSERT em `questions`**, ou seja, após a linha `is_inverted   = EXCLUDED.is_inverted;` (fim do bloco da Task 2):
...
- [ ] **Step 2: Conferir o tamanho do arquivo**

Ler o arquivo. Esperado: ~500-550 linhas. Começa com o header e termina com o `ON CONFLICT (subscale) DO UPDATE` da Task 3.

---

## Task 4: Rodar a migração no Supabase e validar

**Onde:** SQL Editor do Supabase (dashboard web, não local).

- [ ] **Step 1: Abrir o SQL Editor do Supabase**

Navegar para `https://supabase.com/dashboard/project/<seu-projeto>/sql/new`.

- [ ] **Step 2: Copiar o arquivo inteiro e colar no editor**

Conteúdo completo de `sql/2026-04-10-copsoq-redesign.sql`. Clicar em **RUN**.

Expected: mensagem "Success. No rows returned" (ou similar). Se der erro de FK (`departments` não existe), **parar** e avisar que o banco está em estado inesperado — não é algo deste plano consertar.

- [ ] **Step 3: Rodar SELECTs de sanidade (um por vez)**

Cole cada query abaixo no SQL Editor e confira o resultado:

```sql
-- A. As 76 perguntas foram carregadas?
SELECT COUNT(*) AS total, COUNT(*) FILTER (WHERE is_inverted) AS invertidas
FROM questions;
-- Esperado: total = 76, invertidas ≈ 40
```

```sql
-- B. As 29 pgr_actions foram carregadas?
SELECT COUNT(*) FROM pgr_actions;
-- Esperado: 29
```

```sql
-- C. As views novas foram criadas?
SELECT viewname FROM pg_views
WHERE schemaname = 'public'
  AND viewname IN (
    'v_converted_responses','v_subscale_averages','v_pgr_report','v_risk_levels',
    'vw_pgr_completo','vw_resumo_risco_departamento',
    'vw_media_por_categoria_setor','vw_evolucao_mensal'
  )
ORDER BY viewname;
-- Esperado: 8 linhas
```
...
```sql
-- D. A função RPC existe?
SELECT proname FROM pg_proc WHERE proname = 'insert_survey_response';
-- Esperado: 1 linha
```

```sql
-- E. As policies anônimas foram criadas?
SELECT tablename, policyname FROM pg_policies
WHERE tablename IN ('respondents','responses','questions','pgr_actions')
ORDER BY tablename, policyname;
-- Esperado: pelo menos 1 policy de INSERT para anon em respondents e responses
```

```sql
-- F. As tabelas antigas sumiram?
SELECT tablename FROM pg_tables
WHERE schemaname = 'public'
  AND tablename IN ('answers','submissions','employees','categories',
                    'risk_classifications','respostas_brutas','setores',
                    'category_severity','probability_levels','risk_matrix');
-- Esperado: 0 linhas
```

Se **qualquer** SELECT retornar algo diferente do esperado, parar e investigar antes de seguir para o front-end.

- [ ] **Step 4: Smoke test da RPC com dados falsos**

Pegar um `departments.id` real:

```sql
SELECT id, name FROM departments LIMIT 3;
```

Copiar um UUID e rodar:

```sql
-- Substituir <UUID> pelo id copiado acima
SELECT insert_survey_response(
  '<UUID>'::UUID,
  '{"1":4,"2":3,"3":2,"4":5,"5":3,"6":3,"7":2,"8":4,"9":2,"10":1,"11":2,"12":2,"13":4,"14":3,"15":3,"16":2,"17":3,"18":4,"19":4,"20":4,"21":2,"22":3,"23":3,"24":3,"25":3,"26":2,"27":4,"28":3,"29":3,"30":3,"31":3,"32":3,"33":4,"34":4,"35":3,"36":3,"37":3,"38":3,"39":3,"40":2,"41":2,"42":4,"43":4,"44":4,"45":2,"46":3,"47":3,"48":3,"49":4,"50":4,"51":4,"52":4,"53":4,"54":3,"55":3,"56":3,"57":3,"58":3,"59":3,"60":2,"61":3,"62":3,"63":3,"64":2,"65":2,"66":2,"67":3,"68":3,"69":3,"70":3,"71":2,"72":2,"73":1,"74":1,"75":1,"76":1}'::JSONB
);
```

Expected: devolve um UUID do respondente criado.

Conferir que as views já calculam:

```sql
SELECT subscale, media_subescala, probability, severity, risk_grade, risk_classification
FROM v_pgr_report
ORDER BY risk_grade DESC
LIMIT 10;
-- Esperado: 29 linhas (uma por subescala), com medias entre 1 e 5 e classificações calculadas
```

```sql
SELECT grupo_homogeneo, descricao_perigo, classificacao_risco, score_medio
FROM vw_pgr_completo
WHERE total_respondentes > 0
LIMIT 5;
-- Esperado: descricao_perigo com texto real (da pgr_actions), não o nome da subescala
```

Se tudo funcionar, apagar o respondente de teste:

```sql
DELETE FROM respondents WHERE id = '<UUID-devolvido-no-passo-anterior>';
```

- [ ] **Step 5: Commit do arquivo SQL**

```bash
cd "c:/Users/miche/OneDrive/Documentos/treinamento Atigravity"
git add sql/2026-04-10-copsoq-redesign.sql
git commit -m "$(cat <<'EOF'
feat(db): migração COPSOQ II — conecta coleta à análise

- Substitui as duas arquiteturas paralelas (respostas_brutas JSONB
  vs answers/submissions/employees) por um único schema:
  questions, respondents, responses, pgr_actions
- Implementa lógica COPSOQ II: conversão 1-5 com inversão,
  médias por subescala, matriz NR-01 (prob × sev) por setor
- Adiciona catálogo pgr_actions com 29 linhas de perigo →
  consequência → ação de controle para o PGR
- Cria 4 views-ponte (vw_pgr_completo etc.) preservando o shape
  que o dashboard.ts atual espera
- RLS: INSERT anônimo em respondents/responses (survey via QR)

Spec: docs/superpowers/specs/2026-04-10-copsoq-database-redesign.md
...
## Task 5: Atualizar `src/services/survey.ts`

**Files:**
- Modify: `src/services/survey.ts` (arquivo inteiro — 38 linhas hoje)

- [ ] **Step 1: Reescrever o arquivo**

Usar Write para substituir todo o conteúdo por:
...
// setorId continua sendo o UUID de departments (o QR code não muda)
...
- [ ] **Step 2: Rodar o TypeScript compiler para conferir tipos**
...
Expected: nenhum erro (ou só erros em outros arquivos que serão consertados nas próximas tasks).

Se reclamar que `SurveyPage.tsx` usa `currentQuestion.category_name`, está correto — a Task 6 resolve.

---

## Task 6: Ajustar `src/pages/SurveyPage.tsx`

**Files:**
- Modify: [src/pages/SurveyPage.tsx:109](../../../src/pages/SurveyPage.tsx#L109)

- [ ] **Step 1: Trocar `category_name` por `subscale` no ProgressBar**

Usar Edit:

```
old_string:          categoryName={currentQuestion.category_name}
new_string:          categoryName={currentQuestion.subscale}
```

- [ ] **Step 2: Rodar tsc de novo**
...
Expected: nenhum erro em `SurveyPage.tsx`. Pode sobrar erro em `empresas.ts` — a Task 7 resolve.

---

## Task 7: Reescrever 2 funções em `src/services/empresas.ts`

**Files:**
- Modify: [src/services/empresas.ts:41-56](../../../src/services/empresas.ts#L41-L56)

- [ ] **Step 1: Substituir `fetchEmployeesByEmpresa`**
...
// representa um colaborador anônimo que respondeu o questionário.
...
- [ ] **Step 2: Substituir `fetchSubmissionsWithEmpresa`**
...
// É uma "submissão". Reaproveitamos `created_at` como `submitted_at`
...
Expected: build bem-sucedido, sem erros. Gera a pasta `dist/`.

- [ ] **Step 5: Commit das mudanças do front**

```bash
cd "c:/Users/miche/OneDrive/Documentos/treinamento Atigravity"
git add src/services/survey.ts src/pages/SurveyPage.tsx src/services/empresas.ts
git commit -m "$(cat <<'EOF'
feat(front): integração com o schema COPSOQ II novo

- survey.ts: fetchQuestions lê do schema novo (subscale TEXT,
  id SMALLINT, question_text); submitSurveyResponse chama a RPC
  insert_survey_response em vez de gravar JSONB em respostas_brutas
- SurveyPage.tsx: usa question.subscale no ProgressBar
- empresas.ts: fetchEmployeesByEmpresa e fetchSubmissionsWithEmpresa
  agora leem de respondents (as tabelas employees/submissions foram
  dropadas na migração)
...
**Onde:** browser (servidor `npm run dev` local + dashboard do Supabase).
...
Expected: Vite inicia em `http://localhost:5173`.

- [ ] **Step 2: Pegar um UUID de department válido**

No SQL Editor do Supabase:

```sql
SELECT d.id, d.name, e.nome_fantasia
FROM departments d
...
- [ ] **Step 3: Abrir a survey no browser**

`http://localhost:5173/survey?setor=<UUID>`

Expected:
- Tela de boas-vindas aparece
- Ao clicar "Começar", aparece a primeira pergunta ("A sua carga de trabalho acumula-se...")
- O rótulo "Seção:" mostra "Exigências Quantitativas" (Title Case, vindo de `subscale`)

- [ ] **Step 4: Responder todas as 76 perguntas**

Responder qualquer valor Likert em cada uma. No final, a tela de conclusão aparece (com o player de áudio).

- [ ] **Step 5: Verificar no Supabase que os dados chegaram**

```sql
SELECT id, department_id, created_at FROM respondents ORDER BY created_at DESC LIMIT 1;
SELECT COUNT(*) FROM responses WHERE respondent_id = (
  SELECT id FROM respondents ORDER BY created_at DESC LIMIT 1
);
-- Esperado: 1 respondent e 76 responses
```

- [ ] **Step 6: Ver o resultado no Dashboard**

Fazer login como gestor (ou admin) e navegar para:
- `http://localhost:5173/overview` — deve mostrar KPIs com números reais, top riscos e gráfico
- `http://localhost:5173/dashboard` — deve mostrar cards dos setores com classificação (Baixo/Tolerável/Moderado/Significativo/Intolerável)

Expected: nada de "0 colaboradores" ou listas vazias. Os números batem com as respostas submetidas.

- [ ] **Step 7: Gerar o PDF do PGR**

No DashboardPage, clicar em **"Gerar Relatório PGR"**.

Expected: baixa um PDF com:
- Header do sistema
- Resumo executivo
- Tabela com setores e suas subescalas
- Em "Descrição do Perigo" aparece o texto de `pgr_actions.danger_description` (não só o nome da subescala)
- Em "Medidas de Controle" aparece `pgr_actions.control_action` (não o fallback genérico)

- [ ] **Step 8: Verificar que as páginas admin não quebraram**

- `http://localhost:5173/super-admin` — SuperAdminPage deve listar empresas, setores, colaboradores, última submissão sem erro no console
- `http://localhost:5173/setores` — SetoresPage abre normal
- `http://localhost:5173/reports` — ReportsPage abre normal
- `http://localhost:5173/financeiro` — FinanceiroPage abre normal

Expected: nenhuma dessas páginas joga exceção no console.

Se alguma quebrar, **não** consertar nesta task — anotar qual e criar uma task de follow-up.

- [ ] **Step 9: Commit final (se tudo passou)**

Não há mudanças de código a commitar neste passo — só o plano e spec se ainda não foram. Se quiser rastrear a conclusão:

```bash
cd "c:/Users/miche/OneDrive/Documentos/treinamento Atigravity"
git add docs/superpowers/specs/2026-04-10-copsoq-database-redesign.md docs/superpowers/plans/2026-04-10-copsoq-database-redesign.md
git commit -m "$(cat <<'EOF'
docs: spec e plano do redesenho COPSOQ II

Documenta a migração que conecta a coleta à análise e
implementa a lógica COPSOQ II correta (conversão 1-5,
inversão de itens, matriz NR-01 por setor).
...
- **A migração falha no DROP** por causa de alguma view ou tabela não listada que depende do que estamos dropando → usar o `CASCADE` que já está em todos os DROPs.
- **O JOIN `pgr_actions.subscale = questions.subscale` não casa** → conferir caixa do texto (Title Case vs MAIÚSCULAS). Todos os seeds deste plano usam Title Case consistente — se alguém editou depois, `COALESCE` no `vw_pgr_completo` evita NULL e cai no fallback genérico.
- **A RLS bloqueia INSERT anônimo** → conferir que o GRANT EXECUTE foi aplicado e a policy `respondents_anon_insert` existe (Task 4 Step 3 Query E).
- **O `generatePGR.ts` não usa a coluna `consequencias`** (nova) → fica disponível na view; atualizar o TS interface e o PDF é uma melhoria futura, não bloqueia este plano.
- **Schema drift em `empresas`** (colunas `nome_fantasia`/`data_cadastro` existem no código mas não nos SQL antigos) → este plano não toca em empresas. Se a SuperAdminPage quebrar por causa disso na Task 8 Step 8, é um bug pré-existente, não regressão nossa.

---

## Critério de Sucesso

1. Todos os SELECTs de sanidade da Task 4 retornam o esperado.
2. `npm run build` passa sem erros.
3. Um colaborador responde a survey inteira e aparece uma linha em `respondents` + 76 linhas em `responses`.
4. `SELECT * FROM vw_pgr_completo WHERE total_respondentes > 0` devolve linhas com `descricao_perigo` vindo de `pgr_actions` (texto real, não só o nome da subescala).
5. DashboardPage mostra cards reais, OverviewPage mostra KPIs reais, PDF do PGR sai com os textos corretos.
6. Nenhuma página admin que não foi tocada quebra.
